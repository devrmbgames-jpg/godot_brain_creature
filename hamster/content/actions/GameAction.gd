...
signal action_completed(action_type: GameEnums.ActionType)
...
var _character: Node      = null  ## owning GameCharacter
...
func execute(target: Node = null) -> bool:
	if _is_running:
		return false
...
func _brain() -> GameBrain:
	if _character and _character.has_method("get_brain"):
		return _character.get_brain()
	return null

func _attrs() -> GameAttributeContainer:
	if _character and _character.has_method("get_attrs"):
		return _character.get_attrs()
	return null

func _distance_to(target: Node) -> float:
	if _character is Node3D and target is Node3D:
		return (_character as Node3D).global_position.distance_to((target as Node3D).global_position)
	return INF

func _move_toward(target_pos: Vector3, speed: float) -> void:
	if not _character is GameCharacter:
		return
	var c := _character as GameCharacter
